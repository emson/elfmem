...
def _identity_section(name: str) -> str:
    ...
    return (
        ...
        f"1. Run `elfmem recall --frame self` before responding. The SELF "
        f"frame is queryless: it returns the agent's constitution ordered by "
        f"how load-bearing each principle has proven, not blocks matching a "
        f"topic. Passing a query is accepted and ignored.\n"
        f"2. SELF gives you the voice, not the knowledge. For anything "
        f"topical also recall `--frame attention \"<topic>\"`; for "
        f"active-work questions, `--frame task`.\n"
